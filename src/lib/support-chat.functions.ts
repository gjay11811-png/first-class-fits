...
- Contact: hello@firstclassfits.co.

Rules:
- You CANNOT look up a specific order, payment or customer account. For anything order-specific (where is my order, refund status, change address), briefly apologise and direct them to email hello@firstclassfits.co with their order number, or to sign in and open the order.
...
function faqFallback(q: string): string {
  ...
  if (/(ship|deliver|postage|how long|arrive|dispatch)/.test(s))
    return "Shipping is free on orders over £100, otherwise £6.99 standard. UK orders arrive in 2–4 business days (US 3–5), and we dispatch within 48 hours. For anything specific to your order, email hello@firstclassfits.co.";
  if (/(return|refund|exchange|send back)/.test(s))
    return "We offer free 30-day returns on full-price items in original condition; sale items can be exchanged within 14 days. Electronics and worn footwear are exchange-only. To start a return, sign in and open your order, or email hello@firstclassfits.co.";
  ...
  if (/(track|where.*order|order status)/.test(s))
    return "You'll receive a tracking link by email as soon as your order ships, and you can view your orders by signing in. For a specific order, email hello@firstclassfits.co with your order number.";
  if (/(size|sizing|fit|measure)/.test(s))
    return "Sizing details are on each product page. If you're between sizes or unsure, email hello@firstclassfits.co and we'll help — and remember returns are free within 30 days.";
  if (/(contact|email|human|agent|speak|phone)/.test(s))
    return "You can reach our team any time at hello@firstclassfits.co and we'll get back to you quickly.";
  ...
  return "Thanks for your message. I can help with shipping, returns, payment, sizing and orders. For anything specific to your order, email hello@firstclassfits.co and the team will sort it out.";
}
